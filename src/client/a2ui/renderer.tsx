...
export interface A2UIViewerProps {
  ...
  /** Custom ordering of surface IDs (surfaces not in this list appear at the end) */
  surfaceOrder?: string[] | null;
  /** Called when a surface is hidden */
  onHideSurface?: (surfaceId: string) => void;
  /** Called when surface order changes via drag-and-drop */
  onReorderSurfaces?: (newOrder: string[]) => void;
  /** Whether to show surface controls (hide button, drag handle) */
...
export function A2UIViewer({
  ...
  let surfaceList = Array.from(surfaces.values());

  // Filter hidden surfaces
  if (hiddenSurfaces && hiddenSurfaces.length > 0) {
    const hiddenSet = new Set(hiddenSurfaces);
    surfaceList = surfaceList.filter((s) => !hiddenSet.has(s.surfaceId));
  }

  // Apply custom ordering
  if (surfaceOrder && surfaceOrder.length > 0) {
    const orderMap = new Map(surfaceOrder.map((id, idx) => [id, idx]));
    surfaceList.sort((a, b) => {
      const aIdx = orderMap.get(a.surfaceId) ?? Number.MAX_SAFE_INTEGER;
      const bIdx = orderMap.get(b.surfaceId) ?? Number.MAX_SAFE_INTEGER;
      return aIdx - bIdx;
    });
  }
  ...
  if (showControls && onReorderSurfaces) {
    return (
      <DraggableSurfaceList
  ...
  return (
    <div className={`space-y-4 ${className ?? ""}`}>
      {surfaceList.map((surface) => (
        <div key={surface.surfaceId} className="relative group">
          {showControls && onHideSurface && (
            <SurfaceControlBar
              surfaceId={surface.surfaceId}
...
function SurfaceControlBar({
  ...
  return (
    <div className="absolute top-1 right-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
      <button
        onClick={() => onHide(surfaceId)}
        className="p-1 rounded-md bg-white/80 dark:bg-[#12141c]/80 backdrop-blur-sm border border-gray-200/60 dark:border-[#252838] text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"
        title={`Hide ${surfaceId}`}
      >
...
// ─── Draggable surface list (native HTML5 DnD) ───────────────────

function DraggableSurfaceList({
  surfaces,
  onAction,
  onHideSurface,
  onReorderSurfaces,
  ...
  onAction?: A2UIViewerProps["onAction"];
  onHideSurface?: (surfaceId: string) => void;
  onReorderSurfaces: (newOrder: string[]) => void;
  className?: string;
}) {
  ...
  const handleDrop = (e: React.DragEvent, dropIdx: number) => {
    ...
    const newOrder = surfaces.map((s) => s.surfaceId);
    const [removed] = newOrder.splice(dragIdx, 1);
    newOrder.splice(dropIdx, 0, removed);
    onReorderSurfaces(newOrder);
      ...
      {surfaces.map((surface, idx) => (
        <div
          ...
          className={`relative group transition-all ${
            dragOverIdx === idx
              ? "ring-2 ring-amber-400/50 ring-offset-2 dark:ring-offset-[#0a0c12]"
              : ""
          }`}
        >
          ...
          <div className="absolute top-1 right-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
            <span
              className="p-1 rounded-md bg-white/80 dark:bg-[#12141c]/80 backdrop-blur-sm border border-gray-200/60 dark:border-[#252838] text-gray-400 cursor-grab active:cursor-grabbing"
            ...
            {onHideSurface && (
              <button
                onClick={() => onHideSurface(surface.surfaceId)}
                className="p-1 rounded-md bg-white/80 dark:bg-[#12141c]/80 backdrop-blur-sm border border-gray-200/60 dark:border-[#252838] text-gray-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"
                title={`Hide ${surface.surfaceId}`}
              >
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3.98 8.223A10.477 10.477 0 001.934 12C3.226 16.338 7.244 19.5 12 19.5c.993 0 1.953-.138 2.863-.395M6.228 6.228A10.45 10.45 0 0112 4.5c4.756 0 8.773 3.162 10.065 7.498a10.523 10.523 0 01-4.293 5.774M6.228 6.228L3 3m3.228 3.228l3.65 3.65m7.894 7.894L21 21m-3.228-3.228l-3.65-3.65m0 0a3 3 0 10-4.243-4.243m4.242 4.242L9.88 9.88" />
                </svg>
